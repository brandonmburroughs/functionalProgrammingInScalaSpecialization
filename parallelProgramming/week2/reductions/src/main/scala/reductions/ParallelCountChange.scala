...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins)) countChange(money, coins)
      ...
      val (totalWaysLeft, totalWaysRight) = parallel(parCountChange(money - coins(0), coins, threshold),
                            parCountChange(money, coins drop 1, threshold))

      totalWaysLeft + totalWaysRight
    }
  }
