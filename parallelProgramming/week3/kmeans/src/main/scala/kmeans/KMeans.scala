...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.length == 0) means.map(mean => (mean,GenSeq[Point]())).toMap
    else points.map(point => (findClosest(point, means), point)).groupBy(_._1).mapValues(_.map(_._2))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).map{
      case (oldMean: Point, newMean: Point) => oldMean.squareDistance(newMean) <= eta
