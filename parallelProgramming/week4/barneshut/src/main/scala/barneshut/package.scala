...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    ...
    def insert(b: Body): Quad = {
      val newSize = size / 4
      if (size > minimumSize) (bodies :+ b).foldLeft(
        Fork( Empty(centerX - newSize, centerY - newSize, size / 2),
              Empty(centerX + newSize, centerY - newSize, size / 2),
              Empty(centerX - newSize, centerY + newSize, size / 2),
              Empty(centerX + newSize, centerY + newSize, size / 2)
          )
        )(_.insert(_))
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case fork: Fork =>
          ...
          if (fork.size / distance(x, y, fork.massX, fork.massY) < theta) addForce(fork.mass, fork.massX, fork.massY)
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      def getSector(p: Float, min: Float, max: Float): Int = {
        if (p > max) ((max - min) / sectorSize).toInt
        else if (p < min) (min / sectorSize).toInt
        else ((p - min) / sectorSize).toInt
      }

      val x = getSector(b.x, boundaries.minX, boundaries.maxX)
      val y = getSector(b.y, boundaries.minY, boundaries.maxY)
      this(x, y) += b
      this
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      for (cell <- matrix.indices) matrix(cell) = matrix(cell).combine(that.matrix(cell))
      this
    }
