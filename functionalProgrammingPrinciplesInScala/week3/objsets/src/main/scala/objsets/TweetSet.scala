...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    this.left.filterAcc(p, this.right.filterAcc(p, newAcc))
  }
  ...
  def mostRetweeted: Tweet = {
    var mostRetweets = -1
    var mostRetweetedTweet: Tweet = null

    this.foreach(tweet => if (tweet.retweets > mostRetweets) {
      mostRetweets = tweet.retweets
